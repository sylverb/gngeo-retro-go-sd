...
int frame;
/*
 * YM2610 timer slices per video frame. Stock GnGeo uses 256 (very accurate,
 * expensive). Portable targets use fewer slices so a frame stays under ~16 ms.
 * Default 16; override with -DNEO_Z80_SLICES=32 if a title needs tighter timers.
 * (Do not rely on <config.h> here — that resolves to the firmware Odroid
 * config via -I.../Core/Inc/porting.)
 */
#ifndef NEO_Z80_SLICES
#define NEO_Z80_SLICES 16
#endif
int nb_interlace = NEO_Z80_SLICES;
int current_line;
//static int arcade;
//
