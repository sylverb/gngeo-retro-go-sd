...
Uint8 mem68k_fetch_cpu_byte(Uint32 addr) {
    extern Uint8 *neo_vector_patch(void);
    Uint8 *vp = neo_vector_patch();
    addr &= 0xFFFFF;
    /* Offset-xor — must not XOR an odd base pointer (breaks UniBIOS sum8). */
    if (vp && addr < 0x80)
        return vp[addr ^ 1];
    return memory.rom.cpu_m68k.p[addr ^ 1];
}
...
Uint8 mem68k_fetch_bios_byte(Uint32 addr) {
    addr &= 0x1FFFF;
    return memory.rom.bios_m68k.p[addr ^ 1];
}
...
Uint16 mem68k_fetch_sram_word(Uint32 addr) {
    ...
    if (!memory.sram)
        return 0;
    return (memory.sram[addr] << 8) | (memory.sram[(addr + 1) & 0xffff] & 0xff);
}
...
Uint16 mem68k_fetch_ctl1_word(Uint32 addr) {
    /* Big-endian composition — returning 0 made every button look pressed. */
    return (Uint16)((mem68k_fetch_ctl1_byte(addr) << 8) |
                    mem68k_fetch_ctl1_byte(addr + 1));
}

Uint32 mem68k_fetch_ctl1_long(Uint32 addr) {
    return ((Uint32)mem68k_fetch_ctl1_word(addr) << 16) |
           mem68k_fetch_ctl1_word(addr + 2);
}
...
Uint16 mem68k_fetch_ctl2_word(Uint32 addr) {
    return (Uint16)((mem68k_fetch_ctl2_byte(addr) << 8) |
                    mem68k_fetch_ctl2_byte(addr + 1));
}

Uint32 mem68k_fetch_ctl2_long(Uint32 addr) {
    return ((Uint32)mem68k_fetch_ctl2_word(addr) << 16) |
           mem68k_fetch_ctl2_word(addr + 2);
}
...
Uint16 mem68k_fetch_ctl3_word(Uint32 addr) {
    return (Uint16)((mem68k_fetch_ctl3_byte(addr) << 8) |
                    mem68k_fetch_ctl3_byte(addr + 1));
}

Uint32 mem68k_fetch_ctl3_long(Uint32 addr) {
    return ((Uint32)mem68k_fetch_ctl3_word(addr) << 16) |
           mem68k_fetch_ctl3_word(addr + 2);
}
...
Uint16 mem68k_fetch_coin_word(Uint32 addr) {
    return (Uint16)((mem68k_fetch_coin_byte(addr) << 8) |
                    mem68k_fetch_coin_byte(addr + 1));
}
...
void mem68k_store_invalid_byte(Uint32 addr, Uint8 data) {
    /* Watchdog at 0x300001 — do not mask to 20 bits (clears 0x300000). */
    if ((addr & 0xffffffu) == 0x300001u)
        memory.watchdog = 0;
    (void)data;
}
void mem68k_store_invalid_word(Uint32 addr, Uint16 data) {
    if ((addr & 0xfffffeu) == 0x300000u)
        memory.watchdog = 0;
    (void)data;
}
...
void mem68k_store_sram_word(Uint32 addr, Uint16 data) {
    ...
    memory.sram[(addr + 1) & 0xffff] = data & 0xff;
}
...
Uint8 mem68k_fetch_bk_normal_byte(Uint32 addr) {
    ...
    addr &= 0xFFFFF;
    return memory.rom.cpu_m68k.p[(bankaddress + addr) ^ 1];
}
