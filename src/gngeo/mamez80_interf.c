 ...
 * */
	...
	/* No flat 64 KiB mirror — only the 2 KiB work RAM is mutable. */
	mkstate_data(gzf, memory.z80_ram, 0x800, mode);
	if (mode==STREAD) {
		int i;
		for (i = 0; i < 4; i++) {
...
{
    z80_init();
    /* Banked accessors only — do not malloc a 64 KiB mirror (AHB too small). */
    mame_z80mem = NULL;
    if (!memory.rom.cpu_z80.p) {
