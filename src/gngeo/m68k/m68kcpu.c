...
{
#ifdef BUILD_TABLES
  extern int neo_m68k_prepare_jump_table(void);
  static uint emulation_initialized = 0;

  if(!emulation_initialized)
  {
    int prep = neo_m68k_prepare_jump_table();
    if (prep == 0)
      printf("FATAL: m68k jump table alloc failed\n");
    else if (prep == 1)
      m68ki_build_opcode_table(1); /* JT scratch + cycles */
    else if (prep == 2)
      m68ki_build_opcode_table(0); /* JT from flash — cycles still in BSS */
